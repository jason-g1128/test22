---
title: "Spring Cloud 0基础教程"
date: 2020-07-13T16:25:30+08:00
draft: true
---
...
创建一个用于测试的demo案例工程：《http-demo》

创建一个SpringBoot项目,选择依赖

![1584040985888](assets\1584040985888.png)

并在pom.xml中手动添加httpclient的依赖
...
![1584042173999](assets\1584042173999.png)

在test目录下创建HttpTests测试类

![1584042309808](assets\1584042309808.png)
...
在main目录下com.hr.controller包中创建HelloController,代码 如下:
...
依次双击选中HttpTests类中各个方法名,右键选择运行,即可在命令行打印出指定网站和本地controller返回的响应结果

在main目录下com.hr.entity包中创建Student,代码 如下:
...
在HelloTests中添加如下测试方法:

```
@Test
public void testGetStu() throws IOException {
	HttpGet request = new HttpGet("http://localhost:8080/stu");
	String response = this.httpClient.execute(request, new BasicResponseHandler());
	System.out.println(response);
}
```

由于http://localhost:8080/stu返回的是一个Student对象,此处实际得到的是一个json字符串：
...
public class HttpDemoApplicationTests {
	...
	@Test
    public void httpGet() {
        Student user = this.restTemplate.getForObject("http://localhost:8080/stu", Student.class);
        System.out.println(user);
    }
    
    @Test
    public void httpPost() {
        String str = this.restTemplate.postForObject("http://localhost:8080/hello",null, String.class);
...
- 通过RestTemplate的getForObject()/postForObject()方法，传递url地址及实体类的字节码，RestTemplate会自动发起请求，接收响应，并且帮我们对响应结果进行反序列化。
...
Spring Cloud和Dubbo它们两都具备分布式服务治理相关的功能，都能够提供服务注册、发现、路由、负载均衡等。

Dubbo只有Spring Cloud的一部分RPC功能，而Spring Cloud由众多子项目组成，如Spring Cloud Config、Spring Cloud Zuul、Spring Cloud Bus等，提供了搭建分布式系统及微服务常用的工具，如配置管理、服务发现、断路器、智能路由、微代理、控制总线、一次性token、全局锁、选主、分布式会话和集群状态等，满足了构建微服务所需的所有解决方案。
...
mybatis:
  type-aliases-package: com.mall.userservice.pojo
eureka:
  client:
